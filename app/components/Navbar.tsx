  ...
  return (
    <nav className="navbar" aria-label="Main navigation">
      <div className="container nav-container">
        <Link href="/" className="logo" aria-label="WiFi Print Home">
        ...
        <div className={`nav-links ${isMenuOpen ? 'active' : ''}`}>
          ...
          <Link href="/#printing-guide" onClick={() => setIsMenuOpen(false)}>Guide</Link>
          <Link href="/#faq" onClick={() => setIsMenuOpen(false)}>FAQ</Link>
          <ThemeToggle />
          <Link href="/#download" className="btn btn-primary btn-small" onClick={() => setIsMenuOpen(false)}>Download</Link>
        </div>
      </div>
    </nav>
  );
